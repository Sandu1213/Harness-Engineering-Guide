...
| FC-42-07 | pi 的访问日项目资料提供开源项目背景，以及稳定 Prompt、可替换系统提示词的文档入口。 | REF-148，2026-07-26 重读项目 README。 | 只用于定位白盒案例；不证明行为可复现、长期产品结构、本章运行过 pi 或任何安全/质量属性。 |
| FC-42-08 | pi 作者记录系统提示词变化影响工作流的观察、cchistory 追踪、稳定 Prompt 目标，以及每任务五次的可复现评测方法。 | REF-149，2026-07-26 重读作者构建札记。 | 保留作者主语；不写 Terminal-Bench 具体名次或分数，不承诺提示词稳定即可获得确定行为，也不称为本章独立复核。 |
...
- REF-148 与 REF-149 的项目背景、作者观察和评测方法已登记；正文只把它们用作 Prompt 版本语义案例，不替代本章 Manifest、兼容性与回滚证据。
