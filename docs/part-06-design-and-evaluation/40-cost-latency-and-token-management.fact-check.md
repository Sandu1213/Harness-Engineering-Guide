...
| FC-40-10 | pi 的访问日 README 说明其模型注册表从 OpenRouter 与 models.dev 数据生成，并包含价格与能力元数据。 | REF-148，2026-07-26 重读项目 README。 | 可支持 Rate Snapshot 需要明确来源的白盒参照；不摘录任何价格或能力值，不保证数据新鲜、正确或长期结构稳定。 |
| FC-40-11 | pi 作者把 provider 统一层归纳为四种线协议，并记录兼容层差异、跨 provider 上下文交接、中止后部分结果，以及 token/缓存计费只能 best-effort 的经验。 | REF-149，2026-07-26 重读作者构建札记。 | 仅作为作者当时的工程经验；不外推为各供应商当前行为、行业标准、完整兼容清单或本章实测结果。 |
...
- REF-148 与 REF-149 的新增内容只支持“供应商抽象层的现实”小节；项目文档、作者观察与本书 Resource Record 模型保持分层。
