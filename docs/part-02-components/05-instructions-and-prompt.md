...
一份最小的 Prompt 变更记录可以包含组件名称、版本、改动原因、受影响任务、允许样例、拒绝样例、预期输出、验证器范围以及回滚或升级条件。这样，团队修改“每个 `must_fix` 必须附证据”时，可以重跑旧样例并看到哪些报告接口受影响，而不是依靠记忆判断。下一小节把同一要求用到反方向：当变化的不是新增规则而是系统提示词本身的长度时，裁剪同样需要版本与回归证据。

### Harness 税：系统提示词是每次会话都要支付的固定成本

前几节回答的是“这段文字是什么、归谁维护”；本节补充一个预算问题。系统提示词与工具定义在每次会话开始时整体进入上下文，无论本次任务是否用到其中每一条规则。本书由此延伸出“Harness 税”的提法：系统提示词长度是一笔每次会话都要支付的固定 token 成本，应当像其他接口属性一样被显式审查，而不是只增不减地累积。

一个把这笔税压到极低的公开案例是 pi：Mario Zechner（GitHub 用户名 badlogic）开发的开源极简编码代理，2025 年 8 月创建，MIT 协议；2026 年 4 月起仓库迁移至 earendil-works/pi，作者保留项目主导。[REF-148](https://github.com/earendil-works/pi) 它默认只提供四个工具：read（支持文本与图片、offset/limit）、bash（同步执行、可选超时）、edit（精确文本替换）与 write（创建/覆写）；另有默认关闭的只读工具 grep、find、ls，可通过 `--tools` 参数组合出只读模式。这些默认值与参数以访问日（2026-07-26）文档为准。[REF-148](https://github.com/earendil-works/pi) 作者自述，其系统提示词加工具定义合计低于 1000 token，博文附有完整系统提示词全文，只有十几行。[REF-149](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/)

这种极简押注在一个具体判断上：前沿模型已经在训练中内化了编码代理的惯例。作者写道：“Models know how to use bash and have been trained on the read, write, and edit tools with similar input schemas”，因此 Harness 不必用超长系统提示词把这些惯例重新教一遍。[REF-149](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/) 为验证极简方案不会让能力掉档，作者用 Terminal-Bench 2.0 对 pi 做了可复现跑分：每个任务 5 次试验，公开 runner 与结果。本书未核验其名次，正文不引用分数；值得借用的是方法本身——裁剪提示词与增补提示词一样，都应有评测回归兜底，这正是上一节要求的反方向应用。[REF-149](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/)

作为对照，作者观察到 Claude Code 的系统提示词达数千至上万 token，且随版本频繁变更；他为此建立了 cchistory 工具逐版追踪其提示词变化。这些数字与变更频率是作者的观察，本书未独立核验。[REF-149](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/)

把税单摊开到本章的内容分层上，可以看到固定成本集中在哪里。下表是本书的工程扩展：

| 内容类别 | 计费方式 | 应审查的问题 |
| --- | --- | --- |
| 平台或系统约束 | 每次会话固定支付。 | 哪些惯例训练分布可能已覆盖，可以不写？ |
| 工具定义 | 每次会话固定支付。 | 工具数量与描述长度是否都被实际使用？ |
| 项目规则 | 注入即支付，随会话数放大。 | 规则的适用范围是否值得全局注入？ |
| 任务 Brief 与上下文数据 | 随任务变动支付。 | 属于变动成本，不在固定税范围内。 |
| 输出契约 | 随任务与交付接口变动支付。 | 字段要求是否都有验证器消费？ |

在固定成本上，设计选择可以概括为两条路线的权衡。这同样是本书的延伸整理，不是任一来源的原始结论：

| 维度 | 搭训练分布的便车（最小提示词） | 在提示词里重建行为（长提示词） |
| --- | --- | --- |
| 每会话固定成本 | 低；pi 作者自述其系统提示词加工具定义低于 1000 token。 | 高；每条规则与每个工具描述都按会话重复计费。 |
| 依赖的前提 | 模型训练分布已覆盖目标惯例；换模型时前提需重新验证。 | 行为主要由提示词文本承载，对训练分布的假设较少。 |
| 行为可控性 | 未写入的惯例由模型默认行为决定；团队特有规则仍需显式写出。 | 可细致规定流程与措辞，但文本越长，规则冲突与漂移越难审查。 |
| 变更与回归 | 提示词短，diff 可整体审查；裁剪仍需评测回归证明能力不掉档。 | 变更频繁时接口漂移风险高，需要样例与评测持续兜底。 |
| 适用场景 | 与主流编码工具惯例一致的通用任务。 | 团队特有流程、合规要求与模型未见过的领域约定。 |

把这个视角落进本章工作流程的最小做法（本书建议）：

- 盘点：统计当前系统提示词、工具定义与全局注入规则的 token 规模，形成基线记录。
- 分类：为每条规则标注它是训练分布可能已覆盖的通用惯例，还是团队特有要求；只有后者天然值得每次会话付费。
- 裁剪：删除候选内容前，先为受影响任务建立允许样例与评测回归，参照 pi 案例的做法用可复现跑分兜底。
- 记录：把提示词长度与本节两张表的判断写入 Prompt 变更记录，与上一节的最小变更记录合并维护。

最后是边界：pi 的 token 数字是作者自述，token 计数依赖具体分词器与模型；本节不主张“越短越好”，也不据此评价任何产品的设计优劣。当团队无法判断某条惯例是否已被训练分布覆盖时，处理方式与本章其他未知一致：保留该规则并建立样例，或删除并用评测回归验证，而不是靠直觉裁决。
...
- 把系统提示词与工具定义的长度当作每次会话都要支付的固定成本审查；与训练分布重合的惯例可考虑不写，裁剪与增补同样需要评测回归兜底（见“Harness 税”一节）。
...
- REF-148 — pi 仓库 README（earendil-works/pi）；支持本章中 pi 项目背景、默认工具集与只读组合的限定陈述。
- REF-149 — Zechner, *What I learned building an opinionated and minimal coding agent*；支持本章中最小系统提示词、作者论点、Terminal-Bench 可复现跑分方法与作者对 Claude Code 提示词的观察。
