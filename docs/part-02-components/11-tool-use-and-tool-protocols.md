...
### 工具结果的双视图：模型观察投影与人类证据投影

Result Envelope 回答了“结果如何被追溯”，还有一个分层矛盾没有回答：上下文只应放模型需要的，界面只应渲染人需要的，而许多实现只保留一份面向模型的文本。把这份文本同时当作模型的观察与人的证据，通常出现三种症状：

- 为了让人看懂，把渲染后的富内容（表格、差异对比、图像说明）塞进模型上下文，支付模型并不需要的 token；
- 为了节省 token，把结果裁成一句摘要，人失去了核对原始观察的入口；
- 同一段文本既是观察又是证据，没有人能回答“哪些信息在裁剪中丢掉了”。

开源极简编码代理 pi 的统一 LLM 层提供了一个真实实现的参照：工具结果被拆成两份——给模型的 `output`（文本）与给 UI 的 `details`（结构化数据），另支持以原生格式传递附件（如图片）；其作者 Mario Zechner 自述未在其他统一 LLM API 中见过这一抽象。[REF-149](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/) 同一层还在流式输出中对工具调用参数做渐进 JSON 解析（progressive JSON parsing），让 UI 能在调用完成前实时渲染。[REF-149](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/) 这些是该项目的实现细节，字段划分与具体行为以访问日（2026-07-26）资料为准；引用它不代表本书沿用其字段名或运行时。

本书由此延伸出 Result Envelope 的两个投影。这是本书的工程模型，不是 pi 或任何协议的字段定义：

| 维度 | 模型观察投影（Model Observation Projection） | 人类证据投影（Human Evidence Projection） |
| --- | --- | --- |
| 消费者 | 模型上下文与后续判定流程 | 界面、评审者、批准者与审计 |
| 内容取舍 | 只保留下一步判断所需的最小文本 | 保留结构化数据、差异对比、图像等富证据 |
| 成本形态 | 每个 token 都占用上下文窗口 | 不进入上下文，可承载更详尽的内容 |
| 典型失真 | 裁剪过度，丢失可核对的细节 | 渲染过度，掩盖原始观察本身 |
| 关联要求 | 绑定同一调用关联标识 | 绑定同一调用关联标识 |

两个投影必须满足同源约束——它们是同一份 Invocation Record 原始观察的两种派生，而不是两份可以各自演化的真相：

1. **同源派生。** 两个投影都从同一原始观察生成，并绑定同一关联标识；不允许各自独立采集或事后补写。
2. **转换可审查。** 从原始观察到每个投影的裁剪与格式化规则应当可检查，评审者能回答“模型没看到的那部分是什么”。
3. **冲突回到原始观察。** 投影之间或投影与回读不一致时，以原始观察和独立观察为准，而不是更完整或更好看的那个视图。
4. **投影不承载判定。** 无论哪个视图显示“成功”，准入仍由本章的六道门作出；批准、观察与验收仍分别归第 14、15、17 章。

若模型摘要与人批准时看到的证据来自两条独立管线，第 14 章的批准就可能针对一个并不存在的动作，第 15 章也无法评估观察是否被歪曲——这正是同源约束要防止的分叉。落到相邻交接点上，两个投影的分工是：

| 交接点 | 依赖哪一侧 | 原因 |
| --- | --- | --- |
| 第 14 章的人工批准 | 人类证据投影 | 批准者需要完整证据面，而不是模型摘要 |
| 第 15 章的观察质量 | 两个投影的同源性 | 投影分叉时无法判断观察是否被歪曲 |
| 第 17 章的验收 | 原始结果记录与独立观察 | 验收不应建立在任一投影的裁剪结果上 |
| 第 18 章的恢复 | 原始结果记录与关联标识 | 排障与补偿需要回到未裁剪的观察 |

流式的渐进渲染同样只属于人类证据投影的体验层：参数尚未完整解析时，界面可以先行显示，但准入门的时序不变——解析、形状、语义、目标、环境与批准检查仍以完整参数为输入，部分参数不能提前触发执行。同理，任何投影中的“成功”都不缩短“收到成功类返回”与“任务验收”之间的距离。

单一视图并非总是错误：只服务批处理、没有人工界面的系统可以暂时合并两个投影。升级触发条件是出现人工批准或界面审查需求——从那时起，人看到的与模型看到的必须能分别回答“来自哪次观察、裁掉了什么”。
...
结果呈现也属于这种边界分离：模型上下文与人类界面应消费同一 Result Envelope 派生的不同投影，而不是各自维护一份结果（见核心概念中“工具结果的双视图”一节）。
...
当结果需要同时服务模型与人时，同一 Result Envelope 还可以派生模型观察投影与人类证据投影（见“工具结果的双视图”一节）；投影改变呈现，不改变上述证据链。
