...
线性的聊天记录只能回答“后来出现了什么”，却难以表达“当前沿着哪条尝试继续”。
pi 的会话文档提供了一个可检查的参考实现：会话保存为追加式
JSON Lines（JSONL），条目带有 `id` 与 `parentId`，因此历史天然形成一棵树，
当前工作位置则是树上的活动叶节点。以下机制均以访问日
（2026-07-26）的项目文档为准，不外推为所有 Agent 的会话模型。
[REF-152](https://github.com/earendil-works/pi/tree/main/packages/coding-agent/docs)

在这棵树中，会话条目不只包含用户和助手消息。官方文档还描述了模型切换、
思考级别、标签、压缩摘要、分支摘要和扩展状态等条目类型。它们共同记录
“上下文为何变成现在这样”，而不只是保存最终可见文本。

| 公开机制 | 能帮助回答的问题 | 仍不能据此断言什么 |
| --- | --- | --- |
| 追加式 JSONL 与 `id` / `parentId` | 某条记录的父节点是谁，当前路径如何形成？ | 每个条目都正确、完整或已经持久化到安全位置。 |
| 活动叶节点 | 接下来沿哪一条上下文路径继续？ | 这条路径是最优选择或已经通过验收。 |
| 消息、模型、思考级别和标签条目 | 行为变化前后有哪些可见上下文变化？ | 模型内部推理、权限或外部效果已经被完整记录。 |
| 压缩与分支摘要 | 放弃或压缩的路径留下了什么交接材料？ | 摘要没有信息损失，或被放弃路径可以直接重放。 |
| 扩展状态条目 | 扩展需要随会话恢复的状态是什么？ | 任意扩展都安全、兼容或值得信任。 |

pi 文档中的 `/tree`、`/fork` 与 `/clone` 分别服务于树内导航、从旧节点
创建分支，以及把分支路径复制为独立会话。分支摘要则以共同祖先和被放弃
分支为输入，把一份摘要注入新的活动路径。这里的关键不是命令名，而是
“切换路径”和“抹掉历史”并非同一件事：旧路径仍可定位，新路径明确记录
自己从哪里分开。[REF-152](https://github.com/earendil-works/pi/tree/main/packages/coding-agent/docs)

Armin Ronacher 描述过一种实际用法：在支线中让 Agent 处理工具问题，
完成后回到主线，并把支线摘要注入主线继续工作。这是作者对该工作流的观察，
不是本书验证过的恢复保证，也不表示分支中的外部效果会随导航一起回滚。
[REF-151](https://lucumr.pocoo.org/2026/1/31/pi/)

扩展状态也遵循同样的可追溯思路。pi 的扩展可以追加自定义状态条目，并在
会话恢复时重建自己的状态；这让扩展不必把关键状态藏在进程内存里。
但“能被恢复”只表示状态有公开的持久化路径，不等于扩展的状态迁移已经
经过权限审查、版本兼容检查或业务验收。[REF-152](https://github.com/earendil-works/pi/tree/main/packages/coding-agent/docs)

本书由此延伸出一条工作流设计原则：被放弃的尝试应当是可恢复的证据，
而不是被摘要覆盖后无法追问的废料。追加式记录与父指针提供了一个较小的
承诺接口，足以支持分支、回退、派生和审计；至于存储耐久性、访问控制、
隐私、压缩质量与外部效果一致性，仍要由相应环境和验证机制负责。

| 本书审查问题 | 设计上应留下什么 | 缺失时的保守动作 |
| --- | --- | --- |
| 当前路径从哪里分出？ | 当前叶节点、共同祖先和分支理由。 | 不把最新文本当作唯一历史。 |
| 被放弃路径做过什么？ | 尝试、观察、未知效果和分支摘要。 | 先补证，不直接重放动作。 |
| 回到旧节点意味着什么？ | 只改变上下文选择，不声称撤销外部效果。 | 将可能的写入或通知保持为未知。 |
| 扩展怎样恢复状态？ | 版本化状态条目与恢复规则。 | 阻塞自动恢复或要求迁移审查。 |
| 谁能读取整棵树？ | 明确的保存位置、访问边界和保留策略。 | 不把可追溯性误写成默认安全。 |

这种“会话即树”的模型补充了本章前面的 State Record：前者保留路径关系，
后者给出一次执行当前可判断的状态。两者都不能替代第 17 章的结果验收，
也不能单独证明某个分支里的外部动作已成功或已撤销。pi 作者将上下文可检查
和稳定的 Harness 视为重要设计目标，这仍是作者的工程立场，而非通用保证。
[REF-149](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/)
