...
| REF-148 | [pi 仓库 README](https://github.com/earendil-works/pi) | 访问日文档中的无内置权限系统、默认完整执行能力，以及容器或 micro-VM 外置隔离模式。 | 任何隔离配置天然安全、其他 Harness 应采用同一选择，或本章实际运行了这些模式。 |
| REF-149 | [pi 作者构建札记](https://mariozechner.at/posts/2025-11-30-pi-coding-agent/) | 作者对应用层权限提示、代码与执行能力边界及虚假安全感的个人工程立场。 | 无主语的行业结论、取消批准的普遍建议、安全测试结果或其他产品行为。 |
...
| 三种安全架构对照与最小组合建议 | 本书工程比较 | 不属于 pi 的官方安全模型，也不表示任一组合已经部署或验证。 |
