...
MCP 让外部能力可以被描述、发现和调用，但工程上的完成定义更长：候选能力必须有可追溯来源；每次请求必须匹配任务、目标、环境和批准；结果必须被观察；具有外部效果的任务还必须独立验证。将这些边界写成工件，才能让后续的浏览器自动化、协作和 Git 工作流沿用同一套“能力不等于完成”的原则。
...
- [REF-036 与 REF-086 的用途、访问日和外推禁区](24-mcp-and-external-tool-integration.references.md)。
